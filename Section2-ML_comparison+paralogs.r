...
########################################################################## PolyQ ##########################################################################
#Set
  ppi_Qall <- read.csv("./polyQ_paralogs.csv");                                     #Load dataset
  ppi_Qall <- ppi_Qall[, -c(1:2)]                                                   #Delete the AC and polyX positions; not informative
...
#ML - Test
  ...
  #Plot
    ...
    q<- ggroc(list(RF = roc_rfQ, LogReg = roc_logregQ, knn = roc_knnQ, SVM = roc_svmQ, NN = roc_nnQ),legacy.axes = T) +  theme_bw() + geom_segment(aes(x = 0, xend = 1, y = 0, yend = 1),color="darkgrey") +  ggtitle(paste0('A')) + theme(legend.position = c(0.8, 0.3),legend.title = element_blank())  + geom_text(x=0.25, y=0.95, label="polyQ\nInteractome3D + paralogs",color="black")
########################################################################## PolyA ########################################################################## 
#Set
  ppi_Aall <- read.csv("./polyA_paralogs.csv");                                              #Load dataset
  ppi_Aall <- ppi_Aall[, -c(1:2)]                                                   #Delete the AC and polyX positions; not informative
...
#Plot
  ...
  a<- ggroc(list(RF = roc_rfA, LogReg = roc_logregA, knn = roc_knnA, SVM = roc_svmA, NN = roc_nnA),legacy.axes = T) +  theme_bw() + geom_segment(aes(x = 0, xend = 1, y = 0, yend = 1),color="darkgrey") +  ggtitle(paste0('B')) + theme(legend.position = c(0.8, 0.3),legend.title = element_blank())  + geom_text(x=0.25, y=0.95, label="polyA\nInteractome3D + paralogs",color="black")
########################################################################## Plot Both ###################################################################
